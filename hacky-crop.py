...
def export_icon(icon, size, size2, filename, font, color):
    image = Image.new("RGBA", (size2, size), color=(0,0,0,0))
    ...
    draw.text(((size2 - width) / 2, (size - height) / 2), icons[icon],
    ...
    imagemask = Image.new("L", (size2, size), 0)
    drawmask = ImageDraw.Draw(imagemask)

    # Draw the icon on the mask
    drawmask.text(((size2 - width) / 2, (size - height) / 2), icons[icon],
        font=font, fill=255)

    # Create a solid color image and apply the mask
    iconimage = Image.new("RGBA", (size2,size), color)
    ...
    # Create output image
    outimage = Image.new("RGBA", (size2, size), (0,0,0,0))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--size2", type=int, default=16,
            help="Icon size2 in pixels (default: 16)")
    ...
    for icon in selected_icons:
        ...
        print("Exporting icon \"%s\" as %s (%ix%i pixels)" %
                (icon, filename, size, size2))

        export_icon(icon, size, size2, filename, font, color)
